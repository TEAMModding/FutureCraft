...
public abstract class Planet extends CelestialObject {
	...
	public Vec4 atmosphere = new Vec4(1, 1, 1, 0);
	...
	public void render(Camera cam, float time) {
        ...
        if (distance < 5) {
        	...
        	Mat4 model = new Mat4();
        	model = model.multiply(Mat4.translate(PlanetPos.xCoord - cam.pos.xCoord, PlanetPos.yCoord - cam.pos.yCoord, PlanetPos.zCoord - cam.pos.zCoord));
        	...
        	Mat4 realModel = new Mat4();
        	realModel = realModel.multiply(Mat4.translate(PlanetPos.xCoord, PlanetPos.yCoord, PlanetPos.zCoord));
        	realModel = realModel.multiply(Mat4.rotate(this.physical.eqAscNode, 0F, 1F, 0F));
        	realModel = realModel.multiply(Mat4.rotate(this.physical.obliquity, 0F, 0F, 1F));
        	realModel = realModel.multiply(Mat4.rotate(90, 1F, 0F, 0F));
        	realModel = realModel.multiply(Mat4.rotate(-90, 0F, 0F, 1F));
        	realModel = realModel.multiply(Mat4.rotate(-(((time - this.orbit.epoch) / 86400) / this.physical.rotationPeriod * 360) - this.physical.rotationOffset, 0F, 0F, 1F));
        
        	
        	planetShader.bind();
        	planetShader.uniformMat4(model, "model");
        	planetShader.uniformMat4(realModel, "realModel");
        	planetShader.uniformMat4(SpaceRenderer.getViewMatrix(new Vec3(0, 0, 0), cam.rot), "view");
        	...
        	planetShader.uniformVec3(cam.pos, "eyePos");
        	planetShader.uniformVec3(new Vec3(atmosphere.xCoord, atmosphere.yCoord, atmosphere.zCoord), "atmosphereColor");
        	planetShader.uniformFloat((float)atmosphere.wCoord, "atmosphereDensity");
        	...
        	atmosphereShader.uniformMat4(SpaceRenderer.getViewMatrix(new Vec3(0, 0, 0), cam.rot), "view");
        	...
        	atmosphereShader.uniformVec3(new Vec3(atmosphere.xCoord, atmosphere.yCoord, atmosphere.zCoord), "atmosphereColor");
        	atmosphereShader.uniformFloat((float)atmosphere.wCoord, "atmosphereDensity");
        	...
        	if (hasCloudmap) {
        		...
        		cloudShader.uniformMat4(SpaceRenderer.getViewMatrix(new Vec3(0, 0, 0), cam.rot), "view");
        		...
        		cloudShader.uniformVec3(new Vec3(atmosphere.xCoord, atmosphere.yCoord, atmosphere.zCoord), "atmosphereColor");
        		cloudShader.uniformFloat((float)atmosphere.wCoord, "atmosphereDensity");
        	...
        	if (this.ringSize > 0 && this.hasRingmap) {
        		...
        		ringShader.uniformMat4(SpaceRenderer.getViewMatrix(new Vec3(0, 0, 0), cam.rot), "view");
	...
	private void drawPlanet(float scale) {
		...
		for (int i = 0; i < 6; i++) {	
			...
			for (int x = -5; x < 5; x++) {
				for (int y = -5; y < 5; y++) {
					Vec3 corner1 = new Vec3(x, y, 5).normalize();
					Vec3 normal1 = new Vec3(x, y, 5).normalize();
				
					Vec3 corner2 = new Vec3(x + 1, y, 5).normalize();
					Vec3 normal2 = new Vec3(x + 1, y, 5).normalize();
						
					Vec3 corner3 = new Vec3(x + 1, y + 1, 5).normalize();
					Vec3 normal3 = new Vec3(x + 1, y + 1, 5).normalize();
				
				
					Vec3 corner4 = new Vec3(x, y + 1, 5).normalize();
					Vec3 normal4 = new Vec3(x, y + 1, 5).normalize();
				
					glVertex3d(corner1.xCoord, corner1.yCoord, corner1.zCoord);
					glTexCoord2d(x + 5, y + 5);
					glNormal3d(-normal1.xCoord, -normal1.yCoord, -normal1.zCoord);
					glVertex3d(corner2.xCoord, corner2.yCoord, corner2.zCoord);
					glTexCoord2d(x + 5, y + 5);
					glNormal3d(-normal2.xCoord, -normal2.yCoord, -normal2.zCoord);
					glVertex3d(corner3.xCoord, corner3.yCoord, corner3.zCoord);
					glTexCoord2d(x + 5, y + 5);
					...
					glTexCoord2d(x + 5, y + 5);
					glNormal3d(-normal3.xCoord, -normal3.yCoord, -normal3.zCoord);
					glVertex3d(corner4.xCoord, corner4.yCoord, corner4.zCoord);
					glTexCoord2d(x + 5, y + 5);
					glNormal3d(-normal4.xCoord, -normal4.yCoord, -normal4.zCoord);
				}
 			}
			glEnd();
		}
