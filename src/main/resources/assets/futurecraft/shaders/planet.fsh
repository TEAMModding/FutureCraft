...
void main(){
    ...
    vec3 atmoColor = atmosphereColor * atmosphereDensity / cosAtmo * cosTheta;
	vec3 waterColor = isWater * vec3(0, 0, 1) * (lightColor * cosTheta);
	vec3 specular = pow(cosAlpha, 3) * isWater * 0.5 * cosTheta;
    vec3 diffuse = (lightColor * cosTheta) * textureColor;
	vec3 night = nightColor * negCosTheta * nightMultiplier;
    
    gl_FragColor = vec4(diffuse + specular + waterColor + atmoColor + night, 1.0);
}